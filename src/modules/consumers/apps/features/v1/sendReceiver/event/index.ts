...
import { JsonString, SendReceiverConsumerEventDispatcher, WorkerEventDispatcher } from "@kishornaik/utils";
...
const sendReceiverConsumerEventDispatcher=new SendReceiverConsumerEventDispatcher(eventDispatcher);
// #endregion
...
export const sendReceiverConsumerEventSubscriberListener:WorkerEventDispatcher=async ()=>{
  ...
  await sendReceiverConsumerEventDispatcher.subscribe<JsonString>(eventName,async (message)=>{
    ...
    // Get Data
    const request:SenderReceiverRequestDto=JSON.parse(data as JsonString) as SenderReceiverRequestDto;
